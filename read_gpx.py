...
def animate(i, point_gdfs, line_gdfs, timeline, ax, colors):
    ...
    for idx in range(len(point_gdfs)):
        line_subset = line_gdfs[idx][line_gdfs[idx]['time'] <= current_time]
        point_subset = point_gdfs[idx][point_gdfs[idx]['time'] <= current_time]
    ...
    # ctx.add_basemap(ax, source=ctx.providers.Strava.Run)
    # ctx.add_basemap(ax, source=ctx.providers.OpenSeaMap)

def get_borders(geodata: gpd.GeoDataFrame):
    margins = 2.05 # percentage over the actual area  
...
if __name__ == "__main__":
    ...
    ani = animation.FuncAnimation(fig, animate, frames=len(timeline),
                                  ...
                                  interval=10,
                                  repeat=False)
    ctx.add_basemap(ax, source="map_cache/hamburg.tif")
